    ...
    async sendVerificationCode(req, res) {
        try {
            ...
            if (isTestNumber) {
                ...
                // ⭐ ذخیره UTM برای کاربران تست
                if (utmParams && Object.keys(utmParams).some(key => utmParams[key])) {
                    req.session.testUtmParams = {
                        utm_source: utmParams.utm_source || null,
                        utm_medium: utmParams.utm_medium || null,
                        utm_campaign: utmParams.utm_campaign || null
                    };
                    console.log('📊 UTM params saved for test user:', req.session.testUtmParams);
                }
        ...
        try {
            ...
            if (isTestNumber && req.session.testMode) {
                ...
                if (req.session.verificationCode === code &&
                    ...
                    // ⭐ UTM رو نگه‌داریم در session
                    // req.session.testUtmParams قبلا set شده و باقی می‌مونه

                    if (req.session.testUtmParams) {
                        console.log('📊 Test user UTM params preserved:', req.session.testUtmParams);
                    }
    ...
    async checkAuth(req, res) {
        try {
            ...
            if (req.session.isTestUser) {
                console.log('📊 Test user checkAuth - UTM params:', req.session.testUtmParams);
